...
class Informer(nn.Module):
    VALID_ENDPOINTS = ("enc_embedding", 'Mixed_4f', "encoder", 'exchange', 'Mixed_5c', "projection")
    def __init__(self, enc_in, c_out, out_len,
                 factor=5, d_model=512, n_heads=8, e_layers=3, d_layers=2, d_ff=512,
                 dropout=0.0, attn='prob', embed='fixed', freq='h', activation='gelu',
                 output_attention=False, distil=True, mix=True, device=torch.device('cuda:0, 1, 2, 3'), final_endpoint='projection',
                 name='Informer'):
        ...
        self.end_points[end_point] = DataEmbedding(enc_in, d_model, embed, freq, dropout).cuda()
        if self._final_endpoint == end_point:
            return

        end_point = "Mixed_4f"
        self.end_points[end_point] = nn.Conv1d(in_channels=4000, out_channels=832, kernel_size=1, bias=True)
        if self._final_endpoint == end_point:
            return

        end_point = "encoder"
        Attn = ProbAttention if attn == 'prob' else FullAttention
        self.end_points[end_point] = Encoder(
            [
                EncoderLayer(
        ...
        end_point = "exchange"
        self.end_points[end_point] = nn.Conv1d(in_channels=1000, out_channels=500, kernel_size=1, bias=True)
        if self._final_endpoint == end_point:
            return

        end_point = "Mixed_5c"
        self.end_points[end_point] = nn.Conv1d(in_channels=832, out_channels=1024, kernel_size=1, bias=True)
        if self._final_endpoint == end_point:
            return

        end_point = "projection"
        self.end_points[end_point] = nn.Linear(d_model, c_out, bias=True)
    ...
    def forward(self, x, enc_self_mask=None):
        ...
        x = self.end_points["Mixed_4f"](x)
        x, attns = self.end_points["encoder"](x, attn_mask=enc_self_mask)
        x = self.end_points["exchange"](x.permute(0, 2, 1)).permute(0, 2, 1)
        x = self.end_points["Mixed_5c"](x)
        x = self.end_points["projection"](x)
        return x
    ...
    def extract_features(self, x):
        ...
        for end_point in self.VALID_ENDPOINTS:
            if end_point == "enc_embedding":
                x = self.end_points["enc_embedding"](x)
                output_dict[end_point] = x*10
            elif end_point == "Mixed_4f":
                x = self.end_points["Mixed_4f"](x)
                output_dict[end_point] = x.unsqueeze(-1).unsqueeze(-1)*10
            elif end_point == "encoder":
                x, attns = self.end_points["encoder"](x, attn_mask=None)
                output_dict[end_point] = x*10
            elif end_point == "exchange":
                x = self.end_points["exchange"](x.permute(0, 2, 1)).permute(0, 2, 1)
                output_dict[end_point] = x*10
...
class Informer_Backbone(nn.Module):
    def __init__(self, final_endpoint='Mixed_5c', name='Informer_Backbone'):
        ...
        self._model = Informer(final_endpoint=final_endpoint, name=name, enc_in=30, c_out=30, d_model=1000, n_heads=8, e_layers=1, d_layers=1, d_ff=512, factor=5,
                                 distil=False, dropout=0.05, attn="prob", mix=False, output_attention=True, device=torch.device('cuda:0, 1, 2, 3'), out_len=1000)
        self._model.build()

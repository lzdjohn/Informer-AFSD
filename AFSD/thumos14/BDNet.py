...
class I3D_BackBone(nn.Module):
    ...
    def load_pretrained_weight(self, model_path='/home/lzdjohn/AFSD/AFSD30/models/thumos14/checkpoint-15.ckpt'):
    ...
    def forward(self, x):
        # print(self._model.extract_features(x))
...
class BDNet(nn.Module):
    def __init__(self, in_channels=30, backbone_model=None, training=True):
        super(BDNet, self).__init__()
        self.coarse_pyramid_detection = CoarsePyramid([832, 1024])
        # self.coarse_pyramid_detection = CoarsePyramid([512, 512])
        self.reset_params()
        self.backbone = I3D_BackBone(in_channels=in_channels)

        # self.backbone = Exp_Informer(in_channels=in_channels)
        ...
        #
        #     exp = self.backbone(args)  # set experiments

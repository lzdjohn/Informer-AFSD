...
class MLP_(nn.Module):
    ...
    def __init__(self, enc_in, c_out, out_len,
        ...
        end_point = "Mixed_4f"
        self.end_points[end_point] = nn.Linear(4000, 1000, bias=True)
        if self._final_endpoint == end_point:
            return

        end_point = "Mixed_5c"
        self.end_points[end_point] = nn.Linear(4000, 500, bias=True)
        if self._final_endpoint == end_point:
            return
    ...
    def extract_features(self, x):
        ...
        for end_point in self.VALID_ENDPOINTS:
            if end_point == "Mixed_4f":
                ...
                output_dict[end_point] = x_.unsqueeze(-1).unsqueeze(-1)
            elif end_point == "Mixed_5c":
                x = self.end_points["Mixed_5c"](x.permute(0, 2, 1))
                output_dict[end_point] = x.unsqueeze(-1).unsqueeze(-1)
        return output_dict


class MLP_Backbone(nn.Module):
    def __init__(self, final_endpoint='Mixed_5c', name='Informer_Backbone'):
        super(MLP_Backbone, self).__init__()
        self._model = MLP_(final_endpoint=final_endpoint, name=name, enc_in=30, c_out=8, d_model=1000, n_heads=8, e_layers=1, d_layers=2, d_ff=512, factor=5,
